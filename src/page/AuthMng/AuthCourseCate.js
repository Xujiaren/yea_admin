...
class AuthCourseCate extends Component {
    state = {
        visible: false,
        editTitle: '添加',
        isView: false,
    ...
    render() {
        ...
        return (
            <div className="animated fadeIn">
                <Row>
                    <Col xs="12">
                        <Card title={ctype == 18 ? "视频课程分类" : "练习题分类"} style={{ minHeight: '400px' }}>
                            <div className="flex f_row j_space_between align_items mb_10">
                                ...
                                <div>
                                    <Button value={
                                        this.state.ctype == '18'?'authVcate/add':'authTcate/add'
                                    } onClick={this.showModal.bind(this, '', '', '0')}>添加分类</Button>
                                </div>
                            </div>
                            <div style={{ minHeight: '300px' }}>
                                {
                                    this.state.ctype == '18'?
                                    <Table rowKey='categoryId' expandIcon={() => null} columns={ this.col_course } dataSource={this.category_list} pagination={false} />
                                    :
                                    <Table rowKey='categoryId' expandIcon={() => null} columns={ this.col } dataSource={this.category_list} pagination={false} />
                                }
                ...
                <Modal
                    ...
                    footer={this.state.isView ? null : <Button value={
                        this.state.ctype == '18'?'authVcate/edit':'authTcate/edit'
                    } onClick={this._onPublish} type='primary'>提交</Button>}
                >
                    <Form {...formItemLayout}>
    ...
    col_course = [
        ...
        { title: '状态', dataIndex: 'status', key: 'status', render: (item, ele) => ele.status == 1 ? '已启用' : '未启用' },
        {
            ...
            render: (item, ele, index) => (
                <div>
                    <Button value={
                        this.state.ctype == '18'?'authVcate/view':'authTcate/view'
                    }type="primary" className='m_2' size={'small'} onClick={this.showModal.bind(this, "查看", index, ele.parentId)}>
                        查看
                    </Button>
                    <Button value={
                                this.state.ctype == '18'?'authVcate/edit':'authTcate/edit'
                            }type="primary" className='m_2' size={'small'} onClick={this.showModal.bind(this, "修改", index, ele.parentId)}>
                        修改
                    </Button>
                    <Button value={
                                this.state.ctype == '18'?'authVcate/status':'authTcate/status'
                            } onClick={this._onUpdate.bind(this, index)} type={ele.status == 1 ? "danger" : "primary"} size={'small'} >
                        {ele.status == 1 ? '禁用' : '启用'}
                    </Button>
                    <Popconfirm value={
                            this.state.ctype == '18'?'authVcate/del':'authTcate/del'
                        } 
    ...
    col = [
        ...
        {   
            ...
            render: (item, ele, index) => (
                <div>
                    <Button value='authTcate/view' type="primary" size={'small'} className='m_2' onClick={this.showModal.bind(this, "查看", index, ele.parentId)}>查看</Button>
                    <Button value='authTcate/edit' type="primary" size={'small'} className='m_2' onClick={this.showModal.bind(this, "修改", index, ele.parentId)}>修改</Button>
                    <Button value='authTcate/status' className='m_2' onClick={this._onUpdate.bind(this, index)} type={ele.status == 1 ? "danger" : "primary"} size={'small'} >{ele.status == 1 ? '禁用' : '启用'}</Button>
                    <Popconfirm
                        value='authTcate/del' 
                        title={"确定删除吗？"}
                        onConfirm={this._onDelete.bind(this, index)}
...
const LayoutComponent = AuthCourseCate;
