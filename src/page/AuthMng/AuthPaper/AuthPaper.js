...
import { Table,message,Pagination,Switch,Modal,Form,Card,Select ,Input} from 'antd';
...
class AuthPaper extends Component {
    state = {
    ...
    getAuthPaper = ()=>{
        ...
        actions.getAuthPaper({paper_id,keyword, page:this.page_current-1 ,pageSize:this.page_size})
    }
    _onPage = (val)=>{
        return
        const {actions} = this.props;
    ...
    render(){
        ...
        const {importLoading,keyword} = this.state
        return(
            <div className="animated fadeIn">
                <Row>
                    <Col xs="12">
                        <Card title="考题管理" style={{minHeight:'400px'}}>
                            ...
                            <div className="flex f_row j_space_between align_items mb_10">
                                ...
                                <Input.Group compact>
                                    ...
                                    <Button value='authPaper/add' onClick={()=>{
                                        this.props.history.push('/auth/paper/edit/0')
                                    }}>添加试卷</Button>
                                    <Button  value='authPaper/in' onClick={()=>{
                                        this.category_id = 0
                            ...
                            <div className="min_height">
                                <Table rowKey='paperId' columns={this.col} dataSource={this.auth_paper_list} pagination={{
    ...
    col = [
        ...
        { title: '练习题分类', dataIndex: 'categoryId', key: 'categoryId', render:(item,ele)=>this.category_obj[ele.categoryId] },
        ...
        { title: '状态', dataIndex: 'status', key: 'status', render: (item, ele) => ele.status == 1 ? '已启用' : '未启用' },
        {
            ...
            render: (item, ele, index) => (
                <div>
                    <Button  value='authPaper/view' className='m_2' type="primary" size={'small'} onClick={()=>{
                        this.setState({ showTopicPannel:true, topic_list:ele.topicList })
                    }}>查看题目</Button>
                    <Button  value='authPaper/view' className='m_2' type="primary" size={'small'} onClick={()=>{
                        this.props.history.push({
                            pathname:'/auth/paper/edit/'+ele.paperId,
                            state:{type:"view"}
                        })
                    }}>查看</Button>
                    <Button  value='authPaper/edit' className='m_2' type="primary" size={'small'} onClick={()=>{
                        this.props.history.push({
                            pathname:'/auth/paper/edit/'+ele.paperId,
                            state:{type:"edit"}
                        })
                    }}>修改</Button>
                    <Button  value='authPaper/status' className='m_2' onClick={this._onUpdate.bind(this,ele.paperId,'status')} type={ele.status == 0?"primary":'danger'} ghost size={'small'}>{ele.status == 1?'禁用':'启用'}</Button>
                    <Popconfirm
                        value='authPaper/del'
                        okText="确定"
                        cancelText='取消'
...
const LayoutComponent =AuthPaper;
