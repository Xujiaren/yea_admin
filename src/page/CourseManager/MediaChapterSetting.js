...
import {Switch,Popconfirm,Empty,Modal,Form,Upload,Icon,Input,Tree,Card,PageHeader, Button, Select, message, InputNumber} from 'antd';
...
class ChapterSetting extends React.Component {
            ...
            actions.mediaAction({
                ...
                resolved:(data)=>{
                    ...
                    if(duration&&duration!==0)
                        this.PublishChapter({is_free,status,duration,chapter_id,parent_id,media_id,content,chapter_name})
                    else
                        message.info('获取的音频时长为 0，请检查音频链接')
                },
                rejected:(data)=>{
        ...
        if(media_id||media_id !== ''){
            ...
            actions.mediaAction({
                ...
                resolved:(data)=>{
                    let duration = data.duration
                    if(duration&&duration!==0)
                        this.PublishChapter({is_free,status,duration,parent_id,media_id,content,chapter_name})
                    else
                        message.info('获取的音频时长为 0，请检查音频链接')
                },
                rejected:(data)=>{
    ...
    PublishChapter = ({is_free,status,duration,parent_id,media_id,chapter_id,chapter_name,content})=>{

        let course_id = this.course_id
        const {actions} = this.props;
        actions.publishChapter({
            is_free,status,parent_id,media_id,chapter_id,chapter_name,course_id,content,duration,
            resolved:(data)=>{
                this.setState({
                    showEditChapter:false,
                    showEditSection:false,
                                ...
                                >
                                    {ele.courseChapterList === null ?null:ele.courseChapterList.map((_ele,_index)=>(
                                        <TreeNode 
                                            ...
                                            title={`${_ele.chapterName} ${_ele.content} ${_ele.isFree==1?' (试听章节)':''}`}
                                            key={_ele.chapterId} 
                                            isLeaf 
                                        />
                                    ))}
                    ...
                    <Form {...formItemLayout}>
                        ...
                        <Form.Item label="上传音频">
                            ...
                            <div style={{fontSize:'12px',lineHeight:'1.5',marginTop:'-15px'}}>
                                <strong>* 只能上传 MP3 格式的音频文件</strong><br/>
                                * 如上传了音频，音频链接将会自动生成<br/>
                            </div>
                        </Form.Item>
                        <Form.Item label="是否试听">
                            <Switch checked={this.state.is_free == 1} onChange={(e)=>{
                                this.setState({ is_free:e?1:0 })
                            }}></Switch>
                        </Form.Item>
                        <Form.Item label="音频链接">
                            <Input
                                onChange={e=>{
                        ...
                        <Form.Item label="音频时长(秒)">
                            ...
                            * 音频时长取整数<br/>
                            * 如果音频时长为空，点击提交时将会根据音频链接自动获取音频的时长信息，请确保音频链接的正确性，如获取失败，请检查音频链接的有效性<br/>
                            </div>
                        </Form.Item>
