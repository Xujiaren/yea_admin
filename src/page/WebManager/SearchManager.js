...
class SearchManager extends Component {
    ...
    componentDidMount(){
        ...
        actions.getKeyword({action:''})
    }

    componentWillReceiveProps(n_props){
        if(n_props.keyword_list !== this.props.keyword_list){
            this.keyword_list = n_props.keyword_list

            this.setState({
                hot_tags: this.keyword_list.hot,
                default_tags: this.keyword_list.def,
            })
        }
    }
    _onDefSearch =(val)=>{
    ...
    handleClose = (removedTag) => {
        const {actions} = this.props
        actions.removeKeywords({keyword:removedTag,type:'hot',
            resolved:(data)=>{
...
///////
    ...
    d_handleClose = (removedTag) => {
        const {actions} = this.props
        actions.removeKeywords({keyword:removedTag,type:'def',
            resolved:(data)=>{
                message.success('操作成功')
    ...
    d_forMap = (tag,idx) => {
        const tagElem = (
            <Tag
                ...
                closable={this.props.rule.includes('search/del')?true:false}
                onClose={!this.props.rule.includes('search/del')?this.onRefuse:e => {
                    e.preventDefault();
    ...
    render() {
        ...
        <div className="animated fadeIn">
            <Card title="搜索管理" bodyStyle={{paddingTop:"20px"}} extra={<div></div>}>
                <Tabs defaultActiveKey="1">
                    <TabPane tab="热搜管理" key="1">
                        <Card 
                            extra={
                                <Search 
                                    placeholder="搜索"
...
const LayoutComponent =SearchManager;
