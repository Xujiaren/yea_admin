...
import { Table, Radio, Pagination, Switch, Modal, Form, Card, Tabs, Select, Input, Empty, InputNumber, message } from 'antd';
...
class TopicClassify extends Component {
    state = {
        visible: false,
        editTitle: '添加',
        isView: false,
    ...
    render() {
        ...
        return (
            <div className="animated fadeIn">
                <Row>
                    <Col xs="12">
                        <Card title={ctype == 18 ? "课程分类" : "题目分类"} style={{ minHeight: '400px' }}>
                            <Tabs activeKey={this.state.tab} onChange={val => {
                                let pathname = this.props.history.location.pathname
                                this.setState({ tab: val })
                                if(val=='0'){
                                    this.setState({ctype:25},()=>{this.props.actions.getAuthCate({ keyword: this.state.keyword, ctype: this.state.ctype })})
                                }else{
                                    this.setState({ctype:27},()=>{this.props.actions.getAuthCate({ keyword: this.state.keyword, ctype: this.state.ctype })})
                                }
                                ...
                                <TabPane tab="学习地图" key="0">
                                    <div className="flex f_row j_space_between align_items mb_10">
                                        ...
                                        <div>
                                            <Button value='mapTopicCate/add' onClick={this.showModal.bind(this, '', '', '0')}>添加分类</Button>
                                        </div>
                                    </div>
                                    <div style={{ minHeight: '300px' }}>
                                        <Table expandIcon={() => null} rowKey='categoryId' columns={this.state.ctype == 25 ? this.col_course : this.col} dataSource={this.category_list} pagination={false} />
                                    </div>
                                    <div className='mt_10'>{'总共' + this.page_total + '条'}</div>
                                </TabPane>
                                <TabPane tab="课程内链题目分类" key="1">
                                    <div className="flex f_row j_space_between align_items mb_10">
                                        <div>
                                            <Search
                                                placeholder="关键词"
                                    ...
                                    <div style={{ minHeight: '300px' }}>
                                        <Table expandIcon={() => null} rowKey='categoryId' columns={this.state.ctype == 27 ? this.col_course : this.col} dataSource={this.category_list} pagination={false} />
                                    </div>
                                    <div className='mt_10'>{'总共' + this.page_total + '条'}</div>
                                </TabPane>
                            </Tabs>
                ...
                <Modal
                    ...
                    footer={this.state.isView ? null : <Button value='mapTopicCate/edit' onClick={this._onPublish} type='primary'>提交</Button>}
                >
                    <Form {...formItemLayout}>
    ...
    col_course = [
        ...
        { title: '状态', dataIndex: 'status', key: 'status', render: (item, ele) => ele.status == 1 ? '已启用' : '未启用' },
        {
            ...
            render: (item, ele, index) => (
                <div>
                    <Button value='mapTopicCate/view' type="primary" className='m_2' size={'small'} onClick={this.showModal.bind(this, "查看", index, ele.parentId)}>查看</Button>
                    <Button value='mapTopicCate/edit' type="primary" className='m_2' size={'small'} onClick={this.showModal.bind(this, "修改", index, ele.parentId)}>修改</Button>
                    <Button value='mapTopicCate/edit' onClick={this._onUpdate.bind(this, index)} type={ele.status == 1 ? "danger" : "primary"} size={'small'} >{ele.status == 1 ? '禁用' : '启用'}</Button>
                    <Popconfirm
                        value='mapTopicCate/del'
                        title={"该分类里的课程也会被删除，确定删除吗？"}
    ...
    col = [
        { title: 'ID', dataIndex: 'categoryId', key: 'categoryId' },
        { title: '图标', dataIndex: 'link', key: 'link', render: (item, ele) => <img src={ele.link} onClick={() => { this.setState({ showImg: true, url: ele.link }) }} className='head-example-img'></img> },
        { title: '分类', dataIndex: 'categoryName', key: 'categoryName' },
        { title: '状态', dataIndex: 'status', key: 'status', render: (item, ele) => ele.status == 1 ? '已启用' : '未启用' },
        {
            width: '250px',
            ...
            render: (item, ele, index) => (
                <div>
                    <Button value='mapTopicCate/view' type="primary" size={'small'} className='m_2' onClick={this.showModal.bind(this, "查看", index, ele.parentId)}>查看</Button>
                    <Button value='mapTopicCate/edit' type="primary" size={'small'} className='m_2' onClick={this.showModal.bind(this, "修改", index, ele.parentId)}>修改</Button>
                    <Button value='mapTopicCate/edit' className='m_2' onClick={this._onUpdate.bind(this, index)} type={ele.status == 1 ? "danger" : "primary"} size={'small'} >{ele.status == 1 ? '禁用' : '启用'}</Button>
                    <Popconfirm
                        value='mapTopicCate/del'
                        title={"确定删除吗？"}
                        onConfirm={this._onDelete.bind(this, index)}
...
const LayoutComponent = TopicClassify;
