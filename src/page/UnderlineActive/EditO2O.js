...
class EditO2O extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.match.path === '/underactive/view/:id') {
            this.setState({ view_mode: true })
        } else if (this.props.match.path === '/underactive/add/:id') {
        ...
        actions.publishSquadss({
            ...
            resolved: (data) => {
                if (this.flag && this.flag.getValue() == '/I/' && this.flag.getFile() !== '')
                    this.flag.uploadFile(data.squadId, this.props.actions, this, 18)
                else
                    message.success({
                        content: '提交成功',
                        onClose: () => {
    ...
    disabledEndDate = (current) => {
        const { vTime,squad_id } = this.state
        if (!vTime)
            return true
        ...
        if(squad_id==0){
            ...
            return current < endValue.valueOf()-(1000*60*60*24)
        }
                            ...
                            <Col xs="12">
                                ...
                                <Card type='inner'>
                                    <Form {...formItemLayout}>
                                        ...
                                        <Form.Item label="封面" help='332px * 130px'>
                                            <AntdOssUpload
                                        ...
                                        <Form.Item label="发布对象">
                                            <PersonTypePublic ref={ref => this.flag = ref} actions={this.props.actions} courseId={this.state.squad_id} ctype='18' disabled={view_mode} flag={this.state.flag} />
                                        </Form.Item>
                                    </Form>
                                </Card>
