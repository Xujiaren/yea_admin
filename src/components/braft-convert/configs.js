...
const ignoredEntityNodeAttributes = ['style']
...
const htmlToEntity = (options, source) => (nodeName, node, createEntity) => {
	...
	// loop && (meta.loop = loop)
	// poster && (meta.poster = node.getAttribute('poster'))

	// meta.id = node.getAttribute('id')
	// meta.alt = node.getAttribute('alt')
	// meta.title = node.getAttribute('title')
	// meta.controls = node.getAttribute('controls')
	// meta.autoPlay = node.getAttribute('autoPlay')
	// meta.loop = node.getAttribute('loop')
	// meta.poster = node.getAttribute('poster')
	...
	if (nodeName === 'a' && !node.querySelectorAll('img').length) {
		...
		return createEntity('AUDIO', 'MUTABLE', { url: node.getAttribute('src'), meta, nodeAttributes })
	} else if (nodeName === 'video') {
		return createEntity('VIDEO', 'MUTABLE', { url: node.getAttribute('src'), meta, nodeAttributes })
		...
		let se_url = node.getAttribute('data-src')||''

		entityData.url = url||se_url
