...
function defaultRenderTag (props) {
  let {highIndex,tag, key, disabled, onRemove, classNameRemove, getTagDisplayValue, ...other} = props
  return (
    <span style={highIndex==key?{backgroundColor:"#00baff"}:null} data-key={key} key={key} {...other}>
...
class TagsInput extends React.Component {
  ...
  constructor () {
    super()
    this.state = {tag: '', isFocused: false,highIndex:-1}
  ...
  _addTags (tags) {
    let {antd,validationRegex, onChange, onValidationReject, onlyUnique, maxTags, value} = this.props

    if (onlyUnique) {
      tags = uniq(tags)
      tags = tags.filter(tag => value.every(currentTag =>{
        if(this._getTagDisplayValue(currentTag) !== this._getTagDisplayValue(tag)){
          return true;
        }else{
          antd.error("该标签已存在！",0.3);
          return false;
        }
      })
      )
  ...
  searchTag(tag){
      let {value,antd} = this.props;
      let _index = -1;

      if(value.length==0){
        antd.error("没有任何标签！",0.3)
        return;
      }
      value.map((ele,index)=>{
          if(tag==ele){
